...
abstract: >
  Item Response Theory (IRT) is widely applied in the human sciences to model persons’ responses on a set of items measuring one or more dimensions. While several R packages have been developed that implement IRT models, they are each limited to certain prespecified classes of models. Further, most implementations are frequentist while the availability of Bayesian methods remains comparably limited. We demonstrate how to use the R package brms and Stan to specify and fit a wide range of Bayesian IRT models using flexible and intuitive multilevel formula syntax. Further, item and person parameters can be related in both a linear or non-linear manner. Various distributions for categorical, ordinal, and continuous responses are supported. Users may even define their own custom response distribution for use in the presented framework. Common IRT model classes that can be specified natively in the presented framework include 1PL and 2PL logistic models optionally also containing guessing parameters, graded response and partial credit ordinal models, as well as drift diffusion models of response times coupled with binary decisions. Posterior distributions of item and person parameters can be conveniently extracted and post-processed. Model fit can be evaluated and compared using Bayes factors and efficient cross-validation procedures.
keywords:
...
persons’ responses on a set of items measuring one
or more dimensions. Over the years, a multitude of software packages have been
...
more IRT models not natively supported by \pkg{brms}. The non-linear multilevel
formula syntax of \pkg{brms} allows for a flexible yet concise specification of
multidimensional IRT models, with an arbitrary number of person or item
covariates and multilevel structure if required. Prior knowledge can be included
in the form prior distribution, which constitute an essential part of every
Bayesian model. Estimation is performed in \proglang{Stan} using MCMC sampling
via adaptive Hamiltonian Monte Carlo [@hoffman2014; @stanM2019], an efficient
and stable algorithm that works well in high dimensional, highly correlated
parameter spaces. Although the \pkg{brms} framework does not only focus
on IRT models, recent developments and new features presented in this paper
make it particularily appealing for fitting those models.

The paper is structured as follows: In Section \ref{model}, we describe the
model structure of the presented framework and, in Section \ref{brms}, the
...
Further, we provide a comparison of IRT supporting \proglang{R} packages in
...
$D$ is appropriate to model persons responses on the items. In this section, we
will briefly review some common response distributions in IRT that are natively
supported in the proposed framework. A complete overview of all supported
response distributions is provided as a package vignette 
(run `vignette("brms_families")` after loading \pkg{brms}).
In addition, users may also define their own custom response distributions, which
can be made fully compatible with all modeling and post-processing options of
the framework (run `vignette("brms_customfamilies")`).
...
possible response distributions are plausible [for details see @buerkner2019].
...
ordinal families in IRT are the *cumulative* model [@agresti2010], which assumes

$$
P(y = c) = F(\tau_c - \psi) - F(\tau_{c-1} - \psi)
$$
where $F$ is the cumulative distribution function (CDF) of a continuous unbounded distribution and $\tau$ is a vector of $C-1$ ordered thresholds. If $F$ is the standard logistic distribution, the resuling IRT model is called *graded response model* [GRM; @samejima1997]. Alternatively, one can use the *adjacent category model* [ACM; @agresti2010], which, when combined with the logistic distribution, 
...
assessment studies such as PISA [@oecd2017]. We will provides hands-on examples
...
found for instance in @wagenmakers2007 or when running
...
also fosters theoretical understanding on the underlying processes
[@ratcliff1978;
...
the drift rate that describes persons tendency towards one or the other two
...
In the context of IRT, every parameter $\psi_k$ can written as a function
...
Rasch model rather use $\theta_p - \xi_i$, in which case $\xi_i$ becomes the
item difficulty instead of the easiness. Clearly, both formulations are
...
their flexibility comes to a limit. A typical example of such a situation is
...
*products* of parameters. In the context of logistic model for dichotomous
...
In the context of Bayesian IRT, prior distribution may additionally help to identify
...
In the above equation, $x_{jpi}$ is the value of the $j$th predictor for person
...
covariates, the regression coefficients $b_j$ would then recieve additional indices
...
Such items are problematic for the vality of the test as they hinder
...
thus relates the the data to the parameters. It is nothing else than the
...
uncertainty commonly known as 'standard error'). Obtaining the posterior
distribution analytically is rarely possible and thus Bayesian statistics relies
on Markov-Chain Monte-Carlo (MCMC) methods to obtain samples (i.e., random
values) from the posterior distribution. Such sampling algorithms are
computationally very intensive and thus fitting models using full Bayesian inference
is usually much slower than in point estimation techniques. However, advantages of
Bayesian inference -- such as greater modeling flexibility, prior distributions,
and more informative results -- are often worth the increased computational cost
[@gelman2014].
 
In the following, we will explain important aspects in choosing priors for
person parameters, although the same ideas apply to item parameters, as well. A
key decision when setting up an IRT model is whether we want
...
estimated [@gelman2014]. To make a concrete example, we can assume a normal
...
without strongly influecing the obtained posterior distribution. Of course, we
don't need to restrict ourself to normal distributions. Other prior
distributions, such as a student-t distribution are possible as well, although
assuming a normal distribution is arguably the most common choice.
...
hyperparameters. In this, we most commonly use a centered normal distribution so
that
...
for all $\theta_p$, which common standard deviation $\sigma_\theta$, which is
estimated as part of the model. Such a prior implies that parameters are
shrunken somewhat towards their joint mean, a phenomenon also known as *partial
pooling* [@gelmanMLM2006]. Partial pooling makes parameter estimates more robust
as well as less influenced by extreme patterns and noise in the data
...
person and item parameters, we have to slightly amend the model by adding an
...
and item parameters had been (soft) centered around zero. Such a formulation of
IRT models via partially pooled person and/or item parameters moves them into
the framework of *generalized linear multilevel models* (GLMMs) and allows
corresponding GLMM software to fit certain kinds of IRT models [@deboeck2011].

The above model formulation implies that person parameters estimated on different
distributional parameters are assumed independent of each other, which turns
out to be a too restrictive assumptions in many applications. Or, at least,
we do not know a-priori if they are dependent and so it appears to be safer
to allow for possible dependence regardless. Statistically, correlated
person parameters are modeled via a hierarchcial multivariate normal distribution
of the form
...
where $\theta_{kp}$ is the person parameter of person $p$ used in the
prediction of the distributional parameter $\psi_k$ and $\mathbf{\Sigma_\theta}$ 
is the covariance matrix determining both the scale and the dependence structure
of the person parameters. A covariance matrix tends to be relatively hard to
interprete and so it is usually advantagoues to decompose it into a correlation
matrix capturing the dependence structure and a vector of standard deviations
...
and $\mathbf{D}(\sigma_{\theta 1}, \ldots, \sigma_{\theta K})$ denotes
the diagonal matrix with standard deviations $\sigma_{\theta K}$ on the 
diagonal.  Of course, the same argument goes for item parameters estimated on different distributional parameters so that we may want to model
...
standard deviations and correlation matrices. In short, for standard deviation,
we recommend priors whose denisities have a mode at zero and fall off strictily
...
LKJ prior [@lewandowski2009], which can be chosen as uniform over the space of
valid correlation matrices. More details on hyperparameters in \pkg{brms} and
\pkg{Stan} are provided in @brms1, @brms2, and @stanM2019.
...
and improving convergence to dedicated priors for variable selection such as the
horseshoe prior [@carvalho2010; @piironen2017]. Setting priors in general is an
...
The former can take on all real values and as such using the identity link
...
parameters, we need to think of a link function for $\sigma$ as well. This is
...
log link function with corresponding exponential response function. To make this
explicit, we write
...
use in \pkg{brms}. Details on custom families can be found in
\code{vignette("brms\_customfamilies")}.
...
are arbitary and can be freely chosen by the user. If we just predict the main
...
better to more complex models [@gelmanMLM2006]. If, for some reason, partial
...
independent on the mode of the covariate.
...
observations. Depending on the psychometric test, this may be a too restrictive
assumption as items and persons not only differ in the mean response but also in
...
The function `bf` is a shortform for `brmsformula`, which helps setting up
complex models in \pkg{brms}. In the above specification, `par2` and `par3` are
placeholders for the parameter names, which are specific to each response
distribution. Covariates effects on such parameters may be included in the 
same way as before.

The model formulation shown above implies person and item parameters,
respectively, of different distributional parameters to be independent of each
other. However, we would generally prefer to also model correlations between
...
are not necessarily constant across observations, but rather vary across persons
and item. That is, we need to specify a main non-linear formula as well as some
additional linear formulas describing how the non-linear parameters should
predicted. Basically, non-linear parameters are handled in the same way as
distributional parameters. Suppose that `nlpar1` depends on both persons and
items, while `nlpar2` just depends on the items. In \pkg{brms}, we can express
this as
...
non-linear parameters, but instead can specify any number of them in the formula.
Further, the linear predictors of the non-linear parameters may contain
...
unmatched by any regression or IRT framework available in \proglang{R} or
any other freely available programming language.
...
syntax to specify priors in the proposed IRT framework. The priors we choose
as examples below are not ment to imply any practical recommendation for 
exactly these priors. Rather, the prior can only be understood in the
context of the model it is part of [@gelman2017]. Accordingly, user-defined priors 
should always be chosen by keeping the model and relevant subject matter 
knowledge in mind. We will attempt to provide more ideas in this direction in
...
prior on those parameters it automatically set and cannot be changed via the
...
If, for some reasons, one chooses to *not* use partial pooling for the 
item parameters via formulas like `y ~ 0 + item + (1 | person)`,
item parameters will be treated as ordinary regression coeffiecients and
so their prior specification changes too. In this case, we cannot only set
priors on all item parameters at once, but may also on specific items if desired.
In \pkg{brms}, the class referring to regression coefficients if called `"b"`.
That is, we can impose a $\text{Normal}(0, 3)$ prior on all item parameters via
...
The \pkg{brms} package uses \pkg{Stan} [@carpenter2017] on the back-end for the
model estimation. Accordingly, all samplers implemented in \pkg{Stan} can be
used to fit \pkg{brms} models. The flagship algorithm of Stan is an adaptive
Hamiltonian Monte-Carlo (HMC) sampler
[@betancourt2014;@betancourt2017;@stanM2019], which progressed from the
...
than completely independent samples [@gelman2014]. The main drawback of this
...
algorithms applied in \pkg{Stan}, see the \pkg{Stan} user's manual [@stanM2019]
as well as @hoffman2014.
...
priors on the hierarchical standard deviations of person and items parameters.
Given the scale of the logistic response function, this can regarded as
...
models. There a multiple ways to investigate convergence. We could do it
graphically for instance by looking at trace plots (see the right-hand side of
Figure \ref{fig:plot-va-1pl}) or more recently proposed rank plots
[@vehtari2019]. On that basis, we interprete MCMC chains as having converged to
the same target distribution, if the chains are mixing well individually (i.e.,
quickly jumping up and down) and are overlaying each other at the same time
[@gelman2014]. We may also investigate convergence numerically by mean of the
scale reduction factor $\widehat{R}$ [@gelman1992; @gelman2014; @vehtari2019],
which should be close to one (i.e., $\widehat{R} < 1.01$), and the effective
...
interested in. In IRT, this most likely include the person and item parameters,
...
model) in the computation of the individual coefficients. We display item and
person parameters in Figure \ref{fig:coef-item-va-1pl} and
...
ranef_fit_va_1pl <- ranef(fit_va_1pl, robust = TRUE, probs = c(0.05, 0.95))
ranef_fit_va_1pl$id[, , "Intercept"] %>%
	as_tibble() %>%
	rownames_to_column() %>%
	select(-Est.Error) %>%
	arrange(Estimate) %>%
...
endorsed by a lot of persons and thus have strongly positive easieness
...
we don't show their outputs for reasons of brevity. Instead, we directly take a
...
ranef_fit_va_2pl <- ranef(fit_va_2pl, robust = TRUE, probs = c(0.05, 0.95))
ranef_fit_va_2pl$id[, , "eta_Intercept"] %>%
	as_tibble() %>%
	rownames_to_column() %>%
	select(-Est.Error) %>%
	arrange(Estimate) %>%
...
```{r, include=FALSE}
cor_1pl_2pl <- cor(
	ranef_fit_va_1pl$id[, "Estimate", "Intercept"],
	ranef_fit_va_2pl$id[, "Estimate", "eta_Intercept"]
)
...
difference. Similarily, a model inbetween the 1PL and 2PL model, which assumes a
constant discrimination across items, does not improve model fit noticably
either ($\Delta \text{LOOIC} =$ `r round(-2 * loo_1pl_alpha["fit_va_1pl", 1], 2)`; 
$\text{SE} =$ `r round(2 * loo_1pl_alpha["fit_va_1pl", 2], 2)`). For these
reasons, we will continue to use the 1PL model in our further analysis of the
data.

### Modeling Covariates

When analysing the \code{VerbAgg} data set, we are not so much interested
in the item and person parameters themselves, but rather in the
...
difference for which pair of posterior samples. This procedure of transforming
...
As can be seen on the left-hand side of Figure \ref{fig:me-va-cov2}, increased
...
In all of the above described covariate models, there is no particular reasoning
behind the choice of which item or person covariates are assume to vary over
...
model converges just fine (results not displayed here, see LINK for details). Of
course, this is not to say that such a multilevel structure is necessarily
sensible. However, being able to fit those models allows for more principled
ways of testing afterwards *if* the assumed complexity is actually supported by
the data.
...
with a some distractors. As a result, the probability of correctly answering an
...
The items of the \code{VerbAgg} data set have no forced choice response format --
and no right or wrong answers either -- and so modeling guessing probabilities
makes little sense for that data. For reasons of brevity, we are not going to
introduce another data set on which we apply 3PL models, but instead only focus
on showing how to express such a model in \pkg{brms} without actually fitting
the model.
...
of choosing one of the alternatives in case of guessing -- we end up with a
...
There are some important aspects of the above model specification that require
...
the original scale via `inv_logit`. We could have done this directly in the main
formula but this would have implied doing the transformation twice, as `gamma`
appears twice in the formula. For increased  efficiency, we have defined both
`gamma` and `logitgamma` as non-linear parameters and related them via `gamma ~
...
a result [@han2012]. Accordingly, it is probably more favorable to design items
...
decision. However, this is actually not entirely correct as the actual responses
...
In \pkg{brms}, we are not bounded to the reducing the response to a binary
decision and instead can use the full information in the response values
...
$y$ via the GRM, we not only have a predictor term $\eta$, but
also a vector $\tau$ of $C-1$ ordered latent thresholds, where $C$ is the number
of categories ($C = 3$ for the \code{VerbAgg} data). An illustration of the 
model's assumptions is provied in Figure \ref{fig:grm}.
...
model except that we now see two intercepts, which represent the ordinal
thresholds. We do not show their outputs here for reasons of brevity. Instead,
...
theoretically -- `perhaps` is certainly something else than `yes` most
...
ranef_fit_va_ord_1pl <- ranef(fit_va_ord_1pl, robust = TRUE, probs = c(0.05, 0.95))
ranef_fit_va_ord_1pl$id[, , "Intercept"] %>%
	as_tibble() %>%
	rownames_to_column() %>%
	select(-Est.Error) %>%
	arrange(Estimate) %>%
...
df_person_1pl <- as_tibble(cbind(
	mean_va_1pl = ranef_fit_va_1pl$id[, "Estimate", "Intercept"],
	mean_va_ord_1pl = ranef_fit_va_ord_1pl$id[, "Estimate", "Intercept"],
	se_va_1pl = ranef_fit_va_1pl$id[, "Est.Error", "Intercept"],
	se_va_ord_1pl = ranef_fit_va_ord_1pl$id[, "Est.Error", "Intercept"]
))
...
ordinal responses. We had seen in Section \ref{binary} that discriminations were
very similar across items in the binary case and we now want to take a look
again when modeling the full ordinal response. We have to do this with a
slightly different formula syntax, though, as the non-linear syntax of
\pkg{brms} cannot handle the ordinal thresholds in a way that is required when
adding discrimination parameters. However, as having discrimination parameters
in ordinal models is crucial for IRT, \pkg{brms} provides a distributional
parameter `disc` for specifically for that purpose. We can predict this
...
that discrimination do not vary across items in the GRM either.
...
ranef_fit_va_ord_2pl <- ranef(fit_va_ord_2pl, robust = TRUE, probs = c(0.05, 0.95))
ranef_fit_va_ord_2pl$id[, , "Intercept"] %>%
	as_tibble() %>%
	rownames_to_column() %>%
	select(-Est.Error) %>%
	arrange(Estimate) %>%
...
```{r, include=FALSE}
cor_person <- cbind(
	va_1pl = ranef_fit_va_1pl$id[, "Estimate", "Intercept"],
	va_2pl = ranef_fit_va_2pl$id[, "Estimate", "eta_Intercept"],
	va_ord_1pl = ranef_fit_va_ord_1pl$id[, "Estimate", "Intercept"],
	va_ord_2pl = ranef_fit_va_ord_2pl$id[, "Estimate", "Intercept"]
) %>%
	cor() %>%
	round(3)
...
$b_{\rm 1PL} =$ `r bab[, "Estimate"]` ($95\% \; \text{CI} = [$`r bab[, "Q2.5"]`, 
`r bab[, "Q97.5"]`$]$) for the 1PL model and $b_{\rm GRM} =$ 
(`r bao[, "Estimate"]`, $95\% \; \text{CI} = [$`r bao[, "Q2.5"]`, 
`r bao[, "Q97.5"]`$]$) for the GRM, which turns out to be similar.
...
specific effects*, which implies estimating as many regression coefficients
...
smooth and monotonic (up to 180 degrees) but otherwise of unknown functional
...
possible in \pkg{brms}), but this requires a lot more different values of
`rotate`. Thus, for this example, we will just treat `rotate` as a factor and
use dummy coding with $50$ degree as the reference category. Assuming all three
...
will likely result in overfitting. For larger data sets, this option should
definitely be considered more carefully. Since both `sigma`
...
actually the default but we want to make it explicite here). Together
...
predictions (via \code{pp_check(fit_exg1)}; see Figure \ref{tab:pp-exg1}), so we
...
combine them in additively. In contrast, for ability tests, person and item
parameters are assumed to be positive only and combined in multiplicatively
[@vandermaas2011]. The latter can also be expressed an additive relationship on
the log-scale. In the present example, we deal with data of an ability test and
will use the just described log-scale approach.

Again, our interest lies primarily in the effect of the degree of rotation. More
specifically, we are interested which of the three model parameters (drift rate,
...
items (after controling for the rotation), seems to be very small (SD = 
...
each being more or less general in their supported models. An overview of most
of these packages is available on the Psychometrics CRAN task view 
(\url{https://cran.r-project.org/web/views/Psychometrics.html}).
Comparing all of them to \pkg{brms} would be too extensive and barely helpful
for the purpose of the present paper. Accordingly, we focus on a set of seven
widely applied, actively maintained, and sufficiently general packages that can
be used for IRT modeling. These are \pkg{eRm} [@eRm], \pkg{ltm} [@ltm],
\pkg{TAM} [@TAM], \pkg{mirt} [@mirt], \pkg{sirt} [@sirt], \pkg{lme4} [@lme4],
and \pkg{lavaan} [@lavaan]. A high level overview of the modeling options of
each package can be found in Table \ref{tab:pkg-compare} and more details are
provided below.
...
it can fit and the provided estimation algorithms. I also comes with its own
...
purpose of multidimensional IRT models, it does neither support 2PL (or higher)
...
cannot automatically identified from the data structure  (see Section
...
former is fully Bayesian while the latter are mostly based
point estimation methods. \pkg{TAM} and \pkg{mirt} support setting certain prior
distributions on parameters but still perform estimation via optimization.
\pkg{sirt} offers MCMC sampling only for 2PL and 3PL models with
restrictive prior options and almost no built-in methods to post-process
results. While performing full Bayesian inference via MCMC sampling is often
orders of magnitude slower than point estimation via maximum likelihood or least
squares, the obtained information is much higher: Not only do we get the
posterior distribution of all model parameters, but also the posterior
distribution of all quantities that can be computed on their basis [@gelman2014]. 
For instance, the uncertainty in the paramters' posterior naturally propagates to
the posterior predictive distributions, whose uncertainty can then be visualized
along with the mean predictions. \pkg{brms} automates a lot of common
post-processing tasks, such as posterior visualizations, predictions, and model
comparison (see \code{methods(class = "brmsfit")} for a full list of options).
...
publically available IRT software. We have demonstrated its usefulness in
...
the basis for the success of \pkg{brms}. I would also like to thank all the
users who reported bugs or had ideas for new features, thus helping to further
improve \pkg{brms}.
