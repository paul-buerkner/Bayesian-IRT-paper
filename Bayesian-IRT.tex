...
\Plaintitle{Bayesian Item Response Modeling in R with brms and Stan}
\Shorttitle{Bayesian IRT Modeling with \pkg{brms}}
...
\pkg{sirt} \citep{sirt}, and \pkg{psychotree}
...
equation modeling packages such as \pkg{lme4} \citep{lme4}, \pkg{lavaan}
\citep{lavaan}, \pkg{blavaan} \citep{blavaan}, or \pkg{MCMCglmm}
...
\pkg{brms} also provides an opportunity for more methodologically
...
eventually written in \proglang{Stan} itself or in another probabilistic
...
\ref{estimation}, I introduce several new features in \pkg{brms} that
...
supporting \proglang{R} packages in Section \ref{comparison} and end
...
code and examples shown in this paper, \pkg{brms} version 2.11.5 or
higher is required.
...
on the diagonal. Currently, \pkg{brms} officially supports only normal
...
supported in \pkg{brms}. Personally, I prefer hierarchical priors as
...
adopted in \pkg{brms} as well. The procedure is similar to that of
...
In \pkg{brms}, specifying an IRT model is done mainly via three
...
single \proglang{R} formula for the model specification. If we want to
...
The function \code{bf} is a short form for \code{brmsformula}, which
...
\pkg{brms} (and currently unique to it) is to expand the \code{|}
operator into \code{|<ID>|}, where \code{<ID>} can be any value or
symbol. Person or item parameters with the same \code{ID} will then be
...
parsed via standard \proglang{R} formula syntax. Of course, we are not
...
the model structure and generated \proglang{Stan} code. For example, the
\proglang{Stan} code of the 1PL model with non-hierarchical item priors
generated from
...
As can be seen, the \proglang{Stan} code is heavily commented to
facilitate human readability. There are a lot of steps involved in order
to fully generate the \proglang{Stan} code and I will explain the most
important steps below.

In a first step, the information in \texttt{family} and \texttt{formula}
is combined so that all \proglang{R} formulas can be validated against
...
latter. The generated \proglang{Stan} code varies substantially between
...
independently of the individual formulas. The \proglang{Stan} code
...
\texttt{model} block \citep[see][ for more details on \proglang{Stan}
programming blocks]{carpenter2017}. The most important aspect of the
generated \proglang{Stan} code is the use of the non-centered
...
In the \proglang{Stan} code, matrices are split into their column
...
\texttt{parameters} and \texttt{model} blocks of the \proglang{Stan}
...
parameter \texttt{mu}, the following line of \proglang{Stan} code is
generated:
...
\proglang{Stan} expression for the likelihood is found. Specific
combinations of likelihood distribution and link function yield simplied
and more efficient mathematical forms. \proglang{Stan} has several of
...
available, it is used in the generated \proglang{Stan} code. Otherwise,
...
observation. As \proglang{Stan} directly compiles to \proglang{C++},
loops themselves are fast and so the reason for vectorizing in
\proglang{Stan} is different from the reason to vectorize in
\proglang{R} or another interpreted programming language. For the 1PL
model above, the likelihood is written in \proglang{Stan} a follows:
...
Lastly, the priors are added to the \proglang{Stan} code also using the
\texttt{target\ +=} syntax. The \texttt{normal(0,\ 5)} prior on the item
parameters is translated to \proglang{Stan} as follows:
...
\pkg{brms} is that it enables correct marginal likelihood estimation
...
by \proglang{Stan} since \texttt{sd\_1} was defined with a lower
...
algorithm of \proglang{Stan} is an adaptive Hamiltonian Monte-Carlo
...
performance of HMC. The adaptive HMC Sampler of \proglang{Stan} allows
...
in \proglang{Stan}, see the \proglang{Stan} user's manual
...
\code{conditional_effects} method.
...
via various options in \code{mcmc_plot}. After investigating both the
...
\begin{CodeInput}
R> VerbAgg$dif <- as.numeric(with(
R+   VerbAgg, Gender == "F" & mode == "do" & btype %in% c("curse", "scold")
R+ ))
\end{CodeInput}
\end{CodeChunk}
...
a model in \pkg{brms} without actually fitting the model. For an
application of 3PL models to real data using \pkg{brms}, see
...
\citep{sirt}, \pkg{lme4} \citep{lme4}, \pkg{lavaan} \citep{lavaan},
\pkg{blavaan} \citep{blavaan}, and \pkg{MCMCglmm} \citep{MCMCglmm}. All
...
implements exploratoy multilevel IRT models via the \code{mixedmirt}
function. The \pkg{sirt} package, does not provide one single framework
...
particular \pkg{mirt} and \pkg{TAM}. As a result, input and output
structures are not consistent across model fitting functions within
\pkg{sirt}, which makes it more complicated to switch between model
...
functioning via recursive partioning is implemented in \pkg{psychotree}
\citep{psychotree1, psychotree2} based on methods of the
\pkg{psychotools} package \citep{psychotools}. It currently supports
...
estimations. In addition, the \pkg{blavaan} package allows to fit
\pkg{lavaan} models using Bayesian estimation methods. To date, not all
\pkg{lavaan} models are available in \pkg{blavaan}, but I expect this to
change in the future. \pkg{lme4} estimates multilevel models via
...
\pkg{brms} is conceptually closest to \pkg{lme4} and \pkg{MCMCglmm} when
...
\ref{brms}). The multilevel syntax of \pkg{lme4}, \pkg{MCMCglmm}, and
\pkg{brms} allows for an overall shorter model specification than the
structural equation syntax of \pkg{lavaan} as items do not have to be
...
dedicated IRT packages or \pkg{lavaan} syntax.
...
Another difference between \pkg{brms} and several of the other packages
is that the former is fully Bayesian while the latter are mostly based
on point estimation methods. \pkg{TAM} and \pkg{mirt} support setting
certain prior distributions on parameters but still perform estimation
via optimization. \pkg{sirt} offers MCMC sampling only for 2PL and 3PL
models with restrictive prior options and few built-in methods to
post-process results. Similarily, \pkg{blavaan} can fit a subset of the
models supported by \pkg{lavaan} using MCMC methods implemented in
\proglang{JAGS} \citep{jags} or \proglang{Stan} \citep{carpenter2017}
although the set of supported IRT models is currently much smaller than
that of \pkg{brms} (see Table \ref{tab:pkg-compare}). \pkg{MCMCglmm}
...
comparison (see \code{methods(class =
"brmsfit")} for a full list of options and the replication material of
...
provided by \pkg{brms}, \pkg{lme4}, \pkg{lavaan}, or \pkg{MCMCglmm} may
...
\caption{\label{tab:pkg-compare}Overview of modeling options in some IRT supporting packages.}
\centering
\begin{threeparttable}
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{llllllllll}
\toprule
\multicolumn{1}{c}{ } & \multicolumn{9}{c}{Package} \\
\cmidrule(l{3pt}r{3pt}){2-10}
Feature & eRm & ltm & TAM & mirt & sirt & (b)lavaan & lme4 & MCMCglmm & brms\\
\hline
...
had ideas for new features, thus helping to further improve \pkg{brms}.
