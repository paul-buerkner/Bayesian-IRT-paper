...
\citep{lavaan}, or \pkg{blavaan} \citep{blavaan}. I will provide a
...
who strive to develop new IRT models or model variants. \pkg{brms} could
...
\citep{sirt}, \pkg{lme4} \citep{lme4}, \pkg{lavaan} \citep{lavaan}, and
\pkg{blavaan} \citep{blavaan}. All of these packages are of high
...
partioning is implemented in \pkg{psychotree}
...
Another difference between \pkg{brms} and most of the other packages is
that the former is fully Bayesian while the latter are mostly based
point estimation methods. \pkg{TAM} and \pkg{mirt} support setting
...
that of \pkg{brms} (see Table \ref{tab:pkg-compare}). While performing
...
provided by \pkg{brms}, \pkg{lme4}, or \pkg{lavaan} may provide
